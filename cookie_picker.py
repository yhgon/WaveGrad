#### this utility is to handle checkpoint 
#### 1. change iteration number which is related with lr schedule
#### 2. eliminate state_dict for optimizer and amp state 
...
def save_checkpoint_moldelonly(local_rank, model, epoch, total_iter,
                    config, filepath):
    if local_rank != 0:
        return
    print(f"Saving model state at epoch {epoch} to {filepath}")
    checkpoint = {'epoch': epoch,
                  'iteration': total_iter,
                  'config': config,
                  'state_dict': model.state_dict()}
...
def load_checkpoint_modelonly(local_rank, model, epoch, total_iter,
                    config,  filepath, world_size):
    if local_rank == 0:
        print(f'Loading only model state from {filepath}')
    checkpoint = torch.load(filepath, map_location='cpu')
    epoch[0] = checkpoint['epoch'] + 1
    total_iter[0] = checkpoint['iteration']
    config = checkpoint['config']
   ...
   #     print(model) 
    ...
    if local_rank == 0:
        if (epoch % args.epochs_per_checkpoint ==0 ) : 
            ...
            ch_path = os.path.join(args.output, "WaveGrad_model_ch_{:d}.pt".format(epoch) )
            save_checkpoint_modelonly(local_rank, model, epoch,iteration, config, ch_path)            
